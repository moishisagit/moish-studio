...
"""Static registry of external LLM providers. In this fork (Moish seam, Studio slice 1) the
registry holds exactly one entry, the Moish gateway; see ``moish/providers.py``."""
...
# Moish seam (Studio slice 1, Invariant 8): the only provider is Moish. The cloud and
# self-hosted provider entries that stood here are deleted from the fork, not hidden.
from moish.providers import MOISH_PROVIDER
...
def validate_provider_base_url(base_url: str) -> str:
    ...
    # the parsed hostname, so http://provider.example@169.254.169.254/ is caught.
    if not hostname:
